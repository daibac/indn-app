...
$urip_bg_ontablet  = ( $urip_bg_ontablet  != '' ) ? $urip_bg_ontablet  : '';
$urip_bg_onmobile  = ( $urip_bg_onmobile  != '' ) ? $urip_bg_onmobile  : '';
$urip_bg_onphone   = ( $urip_bg_onphone   != '' ) ? $urip_bg_onphone   : '';

if( $urip_bg_ontablet == 'hide' ) {
$css_classes[] = 'row-bg-image-hide-992';
}
if( $urip_bg_onmobile == 'hide' ) {
$css_classes[] = 'row-bg-image-hide-768';
}
if( $urip_bg_onphone == 'hide' ) {
...
if ( $urip_container_display == 'true' ) {
...
if( $urip_vc_row_992_responsive != '' || $urip_vc_row_768_responsive != '' || $urip_vc_row_480_responsive != '' || $urip_bg_position != '' ) {

	$urip_992row=preg_replace('/.vc_custom_[0-9]*/',esc_attr( trim( vc_shortcode_custom_css_class( $css ) ) ),$urip_vc_row_992_responsive);
	$urip_768row=preg_replace('/.vc_custom_[0-9]*/',esc_attr( trim( vc_shortcode_custom_css_class( $css ) ) ),$urip_vc_row_768_responsive);
	$urip_480row=preg_replace('/.vc_custom_[0-9]*/',esc_attr( trim( vc_shortcode_custom_css_class( $css ) ) ),$urip_vc_row_480_responsive);

	$urip_bgpositiony = ( $urip_bg_positiony != '' ) ? $urip_bg_positiony : '';
	$urip_bgpositionx = ( $urip_bg_positionx != '' ) ? $urip_bg_positionx : '';
	$urip_bgxoffset   = ( $urip_bg_xoffset   != '' ) ? $urip_bg_xoffset   : '';
	$urip_bgsize      = ( $urip_bg_size      != '' ) ? $urip_bg_size      : '';

	$urip_bgposition = ( $urip_bg_position == 'custom' ) ? ''.$urip_bg_positiony.' '.$urip_bg_positionx.'' : $urip_bg_position;

	$output .= '<style>';
		if( $urip_vc_row_992_responsive != '' ) {$output .= ' @media only screen and (max-width: 992px) { .'.esc_attr($urip_992row).' }'; }
		if( $urip_vc_row_768_responsive != '' ) {$output .= ' @media only screen and (max-width: 768px) { .'.esc_attr($urip_768row).' }'; }
		if( $urip_vc_row_480_responsive != '' ) {$output .= ' @media only screen and (max-width: 480px) { .'.esc_attr($urip_480row).' }'; }

		if( $urip_bgposition != '' ) {
			$output .= '.vc_row.nt-theme-extra-row-bgposition-'.vc_shortcode_custom_css_class( $css ).'{';
				if( $urip_bgposition != '' ) { $output .= 'background-position:'.$urip_bgposition.' '.$urip_bgxoffset.'!important;'; }
				if( $urip_bgsize     != '' ) { $output .= 'background-size: '.$urip_bg_size.' !important;'; }
			$output .= '}';
		}
	$output .= '</style>';
}
...
// custom line start - for container
if ( $urip_container_display == 'true' ) {
